...
###### The enrichment of drug-gene network hub genes in DORGE-predicted TSG/OGs, NGs, CGC genes and other functional genomics gene sets (or features).
...
###### Output: Figure_4E_pharmacodb_network_hub_enrichment.pdf: drug-gene network hub gene enrichment in DORGE-predicted TSG/OGs, NGs and CGC genes
###### Output: Figure_4F_pharmacodb_network_hub_enrichment.pdf: drug-gene network hub gene enrichment in functional genomics gene sets (or features)
...
######### Figure 4E and 4F: Gene enrichment in Drug-compound network #########
...
drug <- read.table("data/gene_degree_pharmacodb.txt", header=T, fill=TRUE, sep="\t")

quantile_table<-quantile(drug$Degree, prob = seq(0, 1, length = 21), type = 5,na.rm=T) # 95% -> 169

index_high<-which(drug$Degree>as.numeric(quantile_table[length(quantile_table)-1]))
hub_genes<-as.character(drug[index_high,"Gene"])#979
nonhub_genes<-as.character(anno$Gene[-which(anno$Gene%in%hub_genes)])
...
#index_large<-which(Gene_number>100)
#Gene_number <- as.numeric(Gene_number)
#Gene_number[index_large]<-(Gene_number[index_large]-100)/2+100
...
non-Hub	NG
');
df1$genetype<-factor(df1$genetype,levels=c("NG", "CGC-dual", "CGC-OG"
, "CGC-TSG", "Novel DORGE-dual", "Novel DORGE-OG", "Novel DORGE-TSG"))
...
pdf("Raw_figures/Figure_4E_pharmacodb_network_hub_enrichment.pdf", family="ArialMT", width=4.19814*1.12, height=3.0)
p<-ggplot(data=df_2, aes(x=genetype, y=Gene_number, fill=stage))+geom_bar(stat="identity", position=position_dodge())+theme(legend.position="top")+ theme_bw() + theme(axis.ticks.x = element_line(size = 0.5),axis.ticks.y = element_line(size = 0.5),axis.text.y = element_text(hjust = 1, colour = "black"),axis.text.x = element_text(angle = 30, hjust = 1, colour = "black"),plot.title = element_text(hjust = 0.5),legend.title = element_blank(),panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+scale_fill_manual("legend", values = c("Hub" = "#E69F00", "non-Hub" = "grey"))+ labs(x = "", y = "Gene number")+ggtitle("Gene-compound network")+ylim(0,200)
p+geom_signif(y_position=c(max(df_2[13,3],df_2[14,3])+10, max(df_2[5,3],df_2[6,3])+10), xmin=c(0.8,1.8), xmax=c(1.2,2.2),annotation=c(formatC(pval_NG, digits = 2), formatC(pval_CGCdual, format = "e", digits = 2)), tip_length=0.02,size=0.3,textsize =1.7)+geom_signif(y_position=c(max(df_2[3,3],df_2[4,3])+15, max(df_2[1,3],df_2[2,3])+10), xmin=c(2.8,3.8), xmax=c(3.2,4.2),annotation=c(formatC(pval_knownOG, format = "e", digits = 2), formatC(pval_knownTSG, format = "e", digits = 2)), tip_length=0.02,size=0.3,textsize =1.7)+geom_signif(y_position=c(max(df_2[11,3],df_2[12,3])+15, max(df_2[9,3],df_2[10,3])+10), xmin=c(4.8,5.8), xmax=c(5.2,6.2),annotation=c(formatC(pval_prioritized_dual, format = "e", digits = 2), formatC(pval_prioritizedOG, format = "e", digits = 2)), tip_length=0.02,size=0.3,textsize =1.7)+geom_signif(y_position=c(max(df_2[7,3],df_2[8,3])+10), xmin=c(6.8), xmax=c(7.2),annotation=c(formatC(pval_prioritizedTSG, format = "e", digits = 2)), tip_length=0.02,size=0.3,textsize =1.7)
garbage <- dev.off()


pdf("Raw_figures/Figure_4F_pharmacodb_network_hub_enrichment.pdf", family="ArialMT", width=5.2196*1.12, height=3)
p<-ggplot(data=df_3, aes(x=genetype, y=Gene_number, fill=stage))+geom_bar(stat="identity", position=position_dodge())+theme(legend.position="top")+ theme_bw() + theme(axis.ticks.x = element_line(size = 0.5),axis.ticks.y = element_line(size = 0.5),axis.text.y = element_text(hjust = 1, colour = "black"),axis.text.x = element_text(angle = 30, hjust = 1, colour = "black"),plot.title = element_text(hjust = 0.5),legend.title = element_blank(),panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+scale_fill_manual("legend", values = c("Hub" = "#E69F00", "non-Hub" = "grey"))+ labs(x = "", y = "Gene number")+ggtitle("Gene-compound network")+ylim(0,200)
