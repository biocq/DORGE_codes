...
###### Output: Figure_2I_leftpanel.pdf,Figure_2I_rightpanel.pdf: Characterization of top 15 novel predicted genes
###### Output: Figure_2J_leftpanel.pdf,Figure_2J_rightpanel.pdf: Characterization of top 15 novel predicted genes that are undocumented in cancer driver gene databases
...
################################## Figure 2I Top15 non-CGC TSG/OG genes ###############################
...
################################## Figure 2J Find literature untouched TSG/OG genes ###############################
...
pdf(file="Raw_figures/Figure_2J_leftpanel.pdf", family="ArialMT", width=3, height=5)
ha = rowAnnotation(genename = anno_text(rownames(prediction_top_TSG),just="right", location = 1.0, gp = gpar(fontsize = 7)))
...
TSG_sets=list(Predicted_TSG=as.character(TSG_qvalues[TSG_qvalues$TSG_probability>TSG_threshold,1]), CGC_TSG=as.character(TSG_qvalues[TSG_qvalues$TSG_all==1,1]))

Specific_TSG <- setdiff(TSG_sets[[1]],TSG_sets[[2]])
TSG_selected_dat<-TSG_qvalues[which(TSG_qvalues$Gene %in% Specific_TSG),]
...
pdf(file="Raw_figures/Figure_2J_rightpanel.pdf", family="ArialMT", width=5, height=8)
ha = rowAnnotation(genename = anno_text(rownames(prediction_top_OG),just="right", location = 1, gp = gpar(fontsize = 7)))
...
OG_sets=list(Predicted_OG=as.character(OG_qvalues[OG_qvalues$OG_probability> OG_threshold,1]), CGC_OG=as.character(OG_qvalues[OG_qvalues$OG_all==1,1]))

Specific_OG <- setdiff(OG_sets[[1]],OG_sets[[2]])
OG_selected_dat<-OG_qvalues[which(OG_qvalues$Gene %in% Specific_OG),]
